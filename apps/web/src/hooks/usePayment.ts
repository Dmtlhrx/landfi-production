// hooks/usePayment.ts - Fixed version with proper Transaction object handling
...
export const usePayment = () => {
  ...
  const createHederaClient = useCallback((): Client => {
    const network = import.meta.env.HEDERA_NETWORK || 'testnet';
  ...
  const fetchExchangeRate = useCallback(async (): Promise<ExchangeRate> => {
    ...
    try {
      ...
      const defaultRate: ExchangeRate = {
        ...
        network: 'testnet',
        operator_account: import.meta.env.OPERATOR_ACCOUNT 
      };
  ...
  // CORRIGÉ: Process payment with Transaction object (not bytes)
  const processPayment = useCallback(async (
    ...
    try {
      ...
      // IMPORTANT: Créer la transaction et générer l'ID AVANT freezeWith
      ...
      // OBLIGATOIRE: Générer et définir le TransactionId avant freezeWith
      ...
      try {
        // Geler la transaction avec le client Hedera
        ...
        let result;
        try {
          // CORRECTION PRINCIPALE: Passer l'objet Transaction gelé, PAS les octets
          ...
          if (signError.message?.includes('User rejected') || 
            ...
            throw new Error('Format de transaction invalide - Erreur interne corrigée');
          } else {
        ...
        toast.loading('Confirming transaction on network...', { id: 'payment' });
  ...
  }, [walletConnect, connectWallet, canProcessPayment, fetchExchangeRate, api, createHederaClient]);
